 ...
 *
 * All styling uses Wealthsimple brand tokens from tailwind.config.ts.
 */
...
export default function ResultsPanel({ result, onApprove }: Props) {
  ...
  return (
    <div className="border border-ws-border rounded-lg overflow-hidden">
      {/* ── Header bar ───────────────────────────────────────────────── */}
      <div className="bg-light border-b border-ws-border px-5 py-4 flex items-center justify-between gap-4">
      ...
      <div className="bg-light border-t border-ws-border px-5 py-4 flex items-center justify-between gap-4">
        ...
        <button
          onClick={onApprove}
          className="bg-gold text-dune text-xs font-bold tracking-wide uppercase px-4 py-2 rounded hover:bg-opacity-80 transition-opacity shrink-0"
        >
          Approve draft
        </button>
      </div>
